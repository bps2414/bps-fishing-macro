...
import win32api
import win32gui
import winsound  # V3: Sound notifications
...
ctk.set_appearance_mode("dark")  # Dark mode por padrão
...
# Get base directory for file paths (works with PyInstaller .exe)
def get_base_dir():
...
class FishingMacroGUI:
    def __init__(self, root):
        ...
        # Dark mode flag (sempre True com customtkinter)
        self.dark_mode = True
        ...
        self.fishing_end_delay = casting_settings.get('fishing_end_delay', 0.2)  # Delay mínimo seguro
        ...
        self.sound_frequency = sound_settings.get('sound_frequency', 1000)
        self.sound_duration = sound_settings.get('sound_duration', 500)
        ...
        # Novo: opção para desativar rotação/zoom/câmera no modo normal
    ...
    def create_gui(self):
        ...
        # Create scrollable frame usando customtkinter
    ...
    def create_gui_content(self):
        ...
        auto_craft_checkbox = ctk.CTkCheckBox(
            auto_craft_enable_frame,
        ...
        auto_craft_checkbox.pack(pady=15, padx=15)
        ...
        ctk.CTkLabel(
            warning_frame,
            text="⚠️ ATENÇÃO: Configurações Avançadas ⚠️",
            font=("Arial", 12, "bold"),
            text_color="#ff4444"
        ).pack()
        ctk.CTkLabel(
            warning_frame,
            text="Modifique apenas se souber o que está fazendo! Valores incorretos podem causar mal funcionamento.",
            font=("Arial", 9),
            text_color="#ffaa00"
        ).pack(pady=5)
        
        # Checkbox para desativar rotação/zoom/câmera no modo normal
        ...
        disable_normal_camera_checkbox = ctk.CTkCheckBox(
            ...
            text="Desativar rotação/zoom/câmera no modo normal",
        ...
        ctk.CTkLabel(
            disable_camera_frame,
            text="Quando ativado: Desliga rotação de 180°, zoom in/out e inicialização de câmera no modo normal.",
            font=("Arial", 9),
            text_color="#aaaaaa"
        ...
        ctk.CTkLabel(cam_delay_row, text="Delay inicial antes de rotacionar (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(cam_delay_row, text="Padrão: 0.05", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        ...
        ctk.CTkLabel(cam_steps_row, text="Número de passos na rotação:", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(cam_steps_row, text="Padrão: 8 (8x100px = 800px total)", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        ...
        ctk.CTkLabel(cam_step_delay_row, text="Delay entre cada passo (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(cam_step_delay_row, text="Padrão: 0.05", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        ...
        ctk.CTkLabel(cam_settle_row, text="Delay após completar rotação (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(cam_settle_row, text="Padrão: 0.3", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        ...
        ctk.CTkLabel(zoom_ticks_row, text="Número de ticks de zoom:", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(zoom_ticks_row, text="Padrão: 3", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        
        # Zoom tick delay
        zoom_tick_delay_row = ctk.CTkFrame(zoom_frame, fg_color="transparent")
        zoom_tick_delay_row.pack(fill="x", pady=4, padx=15)
        ctk.CTkLabel(zoom_tick_delay_row, text="Delay entre cada tick de zoom (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(zoom_tick_delay_row, text="Padrão: 0.05", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        
        # Zoom settle delay
        zoom_settle_row = ctk.CTkFrame(zoom_frame, fg_color="transparent")
        zoom_settle_row.pack(fill="x", pady=(4, 15), padx=15)
        ctk.CTkLabel(zoom_settle_row, text="Delay após completar zoom (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(zoom_settle_row, text="Padrão: 0.3", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        ...
        ctk.CTkLabel(minigame_wait_row, text="Espera para minigame aparecer (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(minigame_wait_row, text="Padrão: 2.5 (evita detecção prematura)", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        
        # Store click delay
        store_delay_row = ctk.CTkFrame(precast_delays_frame, fg_color="transparent")
        store_delay_row.pack(fill="x", pady=4, padx=15)
        ctk.CTkLabel(store_delay_row, text="Delay após clicar em store (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(store_delay_row, text="Padrão: 1.0", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        
        # Backspace delay
        backspace_delay_row = ctk.CTkFrame(precast_delays_frame, fg_color="transparent")
        backspace_delay_row.pack(fill="x", pady=4, padx=15)
        ctk.CTkLabel(backspace_delay_row, text="Delay após pressionar backspace (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(backspace_delay_row, text="Padrão: 0.3", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        
        # Rod deselect delay
        rod_deselect_row = ctk.CTkFrame(precast_delays_frame, fg_color="transparent")
        rod_deselect_row.pack(fill="x", pady=4, padx=15)
        ctk.CTkLabel(rod_deselect_row, text="Delay após desselecionar rod (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(rod_deselect_row, text="Padrão: 0.5", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        
        # Rod select delay
        rod_select_row = ctk.CTkFrame(precast_delays_frame, fg_color="transparent")
        rod_select_row.pack(fill="x", pady=4, padx=15)
        ctk.CTkLabel(rod_select_row, text="Delay após selecionar rod (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(rod_select_row, text="Padrão: 0.5", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        
        # Bait click delay
        bait_delay_row = ctk.CTkFrame(precast_delays_frame, fg_color="transparent")
        bait_delay_row.pack(fill="x", pady=(4, 15), padx=15)
        ctk.CTkLabel(bait_delay_row, text="Delay após clicar em bait (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(bait_delay_row, text="Padrão: 0.3", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        ...
        ctk.CTkLabel(fruit_det_delay_row, text="Delay durante detecção de fruta (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(fruit_det_delay_row, text="Padrão: 0.02", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        ...
        ctk.CTkLabel(fruit_settle_row, text="Delay após detecção de fruta (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(fruit_settle_row, text="Padrão: 0.3", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        ...
        ctk.CTkLabel(general_delay_row, text="Delay geral entre ações (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(general_delay_row, text="Padrão: 0.05", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        
        # Mouse move settle
        mouse_settle_row = ctk.CTkFrame(detection_frame, fg_color="transparent")
        mouse_settle_row.pack(fill="x", pady=4, padx=15)
        ctk.CTkLabel(mouse_settle_row, text="Delay após mover mouse (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(mouse_settle_row, text="Padrão: 0.05", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
        ...
        ctk.CTkLabel(focus_settle_row, text="Delay após focar Roblox (s):", font=("Arial", 10), width=280, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(focus_settle_row, text="Padrão: 1.0", font=("Arial", 9), text_color="#00ccff").pack(side="left", padx=5)
    ...
    def pre_cast(self):
        ...
        try:
            ...
            if self.auto_store_fruit:
                ...
                # Espera mínima para garantir que o personagem segure a fruta (reduzido para 0.5s)
                print(f"\rPre-cast: Waiting 0.5s for character to hold fruit...", end='', flush=True)
                if not self.interruptible_sleep(0.5):
                    return False
                        ...
                        with mss.mss() as sct:
                            ...
                            if color_match:
                                ...
                                if self.sound_enabled:
                                    try:
                                        threading.Thread(target=lambda: winsound.Beep(self.sound_frequency, self.sound_duration), daemon=True).start()
                                    except:
                                        pass
                ...
                # 8. Click on fruit_point to store (always, regardless of color match)
                if self.fruit_point:
                ...
                # 9. Press backspace to drop/close
    ...
    def capture_auto_craft_area(self):
        """Capture screenshot of the auto craft area using mss"""
        if not self.auto_craft_area_coords:
            return None
    ...
    def capture_area(self):
        """Capture screenshot of the selected area using mss"""
        if not self.area_coords:
            return None
        
        try:
            with mss.mss() as sct:
                # Define monitor area from saved coordinates
                monitor = {
            ...
            if os.path.exists(SETTINGS_FILE):
                with open(SETTINGS_FILE, 'r') as f:
                    ...
                    return {
                        ...
                        'sound_frequency': sound_settings.get('sound_frequency', 1000),
                        'sound_duration': sound_settings.get('sound_duration', 500)
                    }
        except Exception as e:
            logger.error(f"Error loading sound settings: {e}")
        return {'sound_enabled': True, 'sound_frequency': 1000, 'sound_duration': 500}
    ...
    def save_item_hotkeys(self):
        ...
        try:
            ...
            self.info_label.configure(text="✓ Item hotkeys saved!", foreground=self.current_colors['success'])
        except Exception as e:
            self.info_label.configure(text=f"❌ Error saving item hotkeys: {e}", foreground=self.current_colors['error'])
    ...
    def save_disable_normal_camera(self):
        ...
        try:
            ...
            if self.disable_normal_camera:
                self.info_label.configure(text="OK: Camera/zoom/rotation DESATIVADO no modo normal!", text_color="#00dd00")
            else:
                self.info_label.configure(text="OK: Camera/zoom/rotation ATIVADO no modo normal!", text_color="#00dd00")
        except Exception as e:
            self.info_label.configure(text=f"Erro ao salvar: {e}", text_color="#ff4444")
    
    def load_casting_settings(self):
        """Load casting settings from settings file"""
        default_casting = {'cast_hold_duration': 1.0, 'recast_timeout': 30.0, 'fishing_end_delay': 2.0}
        try:
            if os.path.exists(SETTINGS_FILE):
                with open(SETTINGS_FILE, 'r') as f:
                    data = json.load(f)
    ...
    def reset_advanced_settings(self):
        ...
        try:
            ...
            self.info_label.configure(text="✓ Todas configurações resetadas para padrão!", foreground=self.current_colors['success'])
        except Exception as e:
    ...
    def show_black_group_arrow(self, black_group_middle_y):
        ...
        if self.black_group_arrow is None:
            ...
            canvas.create_polygon(
                18, 6,    # Right point (ponta para direita)
                2, 1,     # Top left
                2, 11,    # Bottom left
                fill='blue', outline=''
            )
...
def show_terms_of_use():
    """Show Terms of Use dialog with language selection - returns True if accepted, False if declined"""
    import locale
    
    # Detect system language
    try:
        system_lang = locale.getlocale()[0]
        is_portuguese = system_lang and system_lang.startswith('pt')
    except:
        is_portuguese = False
    
    # Terms of Use text in both languages
    terms_en = """BPS FISHING MACRO - TERMS OF USE 🇧🇷
...
Do you agree to these terms?"""
    ...
    terms_pt = """BPS FISHING MACRO - TERMOS DE USO 🇧🇷

1. PROPÓSITO
Este software é projetado apenas para uso educacional e pessoal.
O usuário assume total responsabilidade pelo seu uso.

2. ISENÇÃO DE RESPONSABILIDADE
- Esta macro é fornecida "COMO ESTÁ" sem garantia de qualquer tipo.
- Os desenvolvedores não são responsáveis por banimentos, suspensões
  ou ações de conta tomadas pelos desenvolvedores do jogo.
- Use por sua própria conta e risco.

3. RESTRIÇÕES
- NÃO venda, redistribua ou reivindique este software como seu.
- NÃO use este software para fins comerciais.
- Respeite os termos de serviço do jogo.

4. CRÉDITOS
• Desenvolvido por: BPS 🇧🇷
• Inspirado por: Trabalho original de Asphalt Cake
• Agradecimentos especiais à comunidade de pesca

Ao clicar em "EU CONCORDO", você reconhece que leu e aceita
estes termos e compreende os riscos envolvidos.

Você concorda com estes termos?"""
    
    terms_text = terms_pt if is_portuguese else terms_en
    btn_agree = "EU CONCORDO" if is_portuguese else "I AGREE"
    btn_decline = "EU RECUSO" if is_portuguese else "I DECLINE"
    ...
    # Spacer para separar botões
    spacer = tk.Frame(dialog, bg='#ecf0f1', height=10)
    spacer.pack(fill=tk.X)
    spacer.pack_propagate(False)
    
    # Buttons frame - SEPARADA E FIXA
