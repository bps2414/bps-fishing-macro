...
# This file is part of BPS Fishing Macro v2.0.0.
#
...
from gui.widgets import CTkToolTip, CollapsibleSidebar
...
class FishingMacroGUI:
    def __init__(self, root):
        ...
        self.discord_bot_auto_menu_channel = discord_bot_settings.get(
            "auto_menu_channel_id", ""
        )

        # v2.0.0: Auto Setup settings
        auto_setup_settings = self.settings.load_auto_setup_settings()
        self.auto_setup_enabled = auto_setup_settings.get("enabled", False)
        self.auto_setup_vip_code = auto_setup_settings.get("vip_code", "")
        self.auto_setup_max_retries = auto_setup_settings.get("max_retries", 5)
        self.auto_setup_game_open_timeout = auto_setup_settings.get(
            "game_open_timeout", 30
        )
        self.auto_setup_game_load_timeout = auto_setup_settings.get(
            "game_load_timeout", 60
        )
        self.auto_setup_play_button = auto_setup_settings.get("play_button", None)
        self.auto_setup_private_server_1 = auto_setup_settings.get(
            "private_server_1", None
        )
        self.auto_setup_vip_code_input = auto_setup_settings.get("vip_code_input", None)
        self.auto_setup_enter_regular = auto_setup_settings.get("enter_regular", None)
        self.auto_setup_menu_color_check = auto_setup_settings.get(
            "menu_color_check",
            {"enabled": True, "coordinate": None, "color_rgb": None, "tolerance": 20},
        )
        self.auto_setup_game_loaded_color_check = auto_setup_settings.get(
            "game_loaded_color_check",
            {"enabled": True, "coordinate": None, "color_rgb": None, "tolerance": 20},
        )
        self.selecting_auto_setup_coord = None  # Track coordinate selection mode
        self.selecting_auto_setup_color = None  # Track color selection mode
        self.auto_setup_running = False  # Track if auto setup is in progress
        self.auto_setup_instance = None  # Track AutoSetup instance
        ...
        self.anti_afk_enabled = False  # Anti-AFK feature flag (not yet implemented)
        ...
        self.webhook_enabled = bool(webhook_settings.get("webhook_url", ""))  # True if URL is configured
        ...
        if self.discord_bot_enabled and encrypted_token and self.discord_bot_app_id:
            try:
                ...
                # Update bot status label and checkbox after GUI is ready
                def update_bot_ui_status():
                    if (
                        hasattr(self, "bot_status_label")
                        and self.bot_status_label.winfo_exists()
                    ):
                        if self.bot_service and self.bot_service.is_running():
                            self.bot_status_label.configure(
                                text="🟢 Online", text_color="#00dd00"
                            )
                    # Ensure checkbox reflects enabled state
                    if hasattr(self, "discord_bot_enabled_var"):
                        self.discord_bot_enabled_var.set(True)

                self.root.after(2000, update_bot_ui_status)
            except Exception as e:
        ...
        logger.info(f"=== BPS Fishing Macro v2.0.0 ===")
    ...
    def _execute_bot_command(self, cmd_data):
        ...
        try:
            if command == "start":
                if not self.running and not self.auto_setup_running:
                    # Use toggle_start to respect Auto Setup settings
                    self.toggle_start()

                    # Check if macro started or auto setup started
                    if self.running:
                        self._send_bot_response(
                            channel_id, f"✅ **Macro started by {author_name}**"
                        )
                    elif self.auto_setup_running:
                        self._send_bot_response(
                            channel_id, f"🚀 **Auto Setup started by {author_name}**"
                        )
                    else:
                        self._send_bot_response(channel_id, "❌ **Failed to start**")
                else:
                    status = "running" if self.running else "setting up"
                    self._send_bot_response(
                        channel_id, f"⚠️ **Macro is already {status}**"
                    )
                ...
                confirm_token = command_data.get("confirm", "").upper()
                if confirm_token != "CONFIRM":
                    self._send_bot_response(
                        channel_id,
                        "⚠️ **Confirmation required!**\nUse: `!shutdown CONFIRM` to proceed.",
                    )
                    return
                self._send_bot_response(
                    channel_id,
                ...
                def _do_shutdown():
                    try:
                        ...
                        self._send_bot_response(
                            channel_id, f"❌ **Shutdown failed:** {e}"
                        )
...
"""
        ...
        try:
            ...
            if self.bot_service and self.bot_service.is_running():
                ...
                if loop:
                    ...
                    logger.info(
                        f"[BOT] Screenshot sent to channel {channel_id} (auto-delete in 60s)"
                    )
    ...
    def _send_bot_response(
        self, channel_id: int, message: str, delete_after: float = 10.0
    ):
                ...
                if loop:
                    asyncio.run_coroutine_threadsafe(
                        self.bot_service.send_message(
                            channel_id, message, delete_after=delete_after
                        ),
                        loop,
                    )
            except Exception as e:
    ...
    def _handle_eta_command(self, channel_id: int):
        ...
        try:
            ...
            if fish_rate == 0:
                self._send_bot_response(
                    channel_id, "⚠️ **Not enough data — catch some fish first!**"
                )
                return
...
📊 Fruits caught: **{self.fruits_caught}** (avg {int(avg_fish_per_fruit)} fish/fruit)"""
    ...
    async def _send_discord_menu(self, channel_id: int):
        ...
        try:
            ...
            if not screenshot_file:
                ...
                try:
                    ...
                    with mss.mss() as sct:
                        ...
                        temp_dir = Path(tempfile.gettempdir())
                        screenshot_path = (
                            temp_dir / f"menu_fallback_{int(time.time())}.png"
                        )

                        # Convert to standard format
                        mss.tools.to_png(
                            sct_img.rgb, sct_img.size, output=str(screenshot_path)
                        )
                        screenshot_file = str(screenshot_path)
            ...
            embed.add_field(
                name="⏱️ Avg Cycle",
                value=(
                    f"{self.average_cycle_time:.1f}s"
                ...
                async def refresh_btn(
                    ...
                    try:
                        ...
                        if hwnd:
                            ...
                            if screenshot is not None:
                                print(
                                    "[MENU-REFRESH] Screenshot captured successfully!"
                                )
                                import cv2
                        ...
                        if not screenshot_file:
                            ...
                            try:
                                ...
                                with mss.mss() as sct:
                                    ...
                                    screenshot_path = (
                                        temp_dir
                                        / f"menu_fallback_{int(time.time())}.png"
                                    )
                                    ...
                                    screenshot_file = str(screenshot_path)
                                    print(
                                        f"[MENU-REFRESH] Fallback successful: {screenshot_file}"
                                    )
                            except Exception as e:
                        ...
                        new_embed.add_field(
                            name="🎣 Casts",
                            value=str(
                                self.app.total_casts
                                if hasattr(self.app, "total_casts")
                                else 0
                            ),
                            inline=True,
                        )
                        new_embed.add_field(
                            name="⏱️ Avg Cycle",
                            value=(
                                f"{self.app.average_cycle_time:.1f}s"
                                if self.app.average_cycle_time > 0
                                else "N/A"
                            ),
                            inline=True,
                        )
        ...
        try:
            ...
            if (
                ...
                if loop and loop.is_running():
                    logger.info(
                        f"[BOT] Menu - Loop is running, scheduling async menu send"
                    )
    ...
    def toggle_discord_bot(self):
        ...
        if enabled:
            ...
            guild_id = (
                self.bot_guild_id_entry.get().strip()
                if hasattr(self, "bot_guild_id_entry")
                else ""
            )
            auto_menu_channel_id = (
                self.bot_auto_menu_channel_entry.get().strip()
                if hasattr(self, "bot_auto_menu_channel_entry")
                else ""
            )
                ...
                def start_bot_async():
                    try:
                        ...
                        def update_success_status():
                            ...
                            if (
                                hasattr(self, "info_label")
                                and self.info_label.winfo_exists()
                            ):
                                self.info_label.configure(
                        ...
                        self.root.after(2000, update_success_status)
                        ...
                        self.root.after(100, update_error_status)
            ...
            try:
                ...
                async def get_channels():
                    ...
                    try:
                        ...
                        text_channels = [
                            ch
                            for ch in channels
                            if hasattr(ch, "topic") or str(ch.type) == "text"
                        ]
                    except Exception:
                ...
                if loop and loop.is_running():
                    ...
                    if result:
                        ...
                        self.root.after(
                            0, lambda: self._show_channels_window(guild, channels)
                        )
                    else:
                        self.root.after(
                            0,
    ...
    def _safe_update_widget(self, widget_attr, configure_params):
        """Safely update a widget if it exists (handles destroyed widgets in sidebar navigation)

        Args:
            widget_attr: Name of the widget attribute (e.g., 'info_label')
            configure_params: Dict of parameters to pass to configure()
        """
    ...
    def create_gui_content(self):
        ...
        self.sidebar = CollapsibleSidebar(main_container, self)
        self.sidebar.pack(side="left", fill="y", padx=(0, 10))

        # Configure sidebar categories
        self.sidebar.add_category(
            "General", "🏠", callback=lambda: self.load_tab("general")
        )
        self.sidebar.add_category(
            "Settings",
            "⚙️",
            sub_items=[
                {"name": "Settings", "callback": lambda: self.load_tab("settings")},
                {"name": "Fishing", "callback": lambda: self.load_tab("fishing")},
                {"name": "Auto Craft", "callback": lambda: self.load_tab("auto_craft")},
                {"name": "Advanced", "callback": lambda: self.load_tab("advanced")},
            ],
        )
        self.sidebar.add_category(
            "Integrations",
            "🔗",
            sub_items=[
                {"name": "Webhooks", "callback": lambda: self.load_tab("webhooks")},
                {"name": "Discord", "callback": lambda: self.load_tab("discord")},
            ],
        )
        self.sidebar.add_category(
            "Stats", "📊", callback=lambda: self.load_tab("stats")
        )
        self.sidebar.add_category(
            "Logging", "📋", callback=lambda: self.load_tab("logging")
        )
        self.sidebar.add_category(
            "Auto Setup", "🚀", callback=lambda: self.load_tab("auto_setup")
        )

        # Right: Content area for dynamic tab loading
        self.content_frame = ctk.CTkFrame(main_container, fg_color="transparent")
        self.content_frame.pack(side="left", fill="both", expand=True)

        # Load default tab (General)
        self.load_tab("general")
    ...
    def load_tab(self, tab_name):
        """Load a tab into the content area

        Args:
            tab_name: Name of tab to load (general, settings, fishing, auto_craft, webhooks, discord, advanced, stats, logging)
        """
        # Create new frame FIRST (not packed yet, so invisible)
        new_tab_frame = ctk.CTkFrame(self.content_frame, fg_color="transparent")

        # Build tab content in the new frame
        try:
            if tab_name == "general":
                general_tab.build(self, new_tab_frame)
            elif tab_name == "settings":
                settings_tab.build(self, new_tab_frame)
            elif tab_name == "fishing":
                fishing_tab.build(self, new_tab_frame)
            elif tab_name == "auto_craft":
                craft_tab.build(self, new_tab_frame, ensure_ocr_dependencies)
            elif tab_name == "webhooks":
                webhook_tab.build(self, new_tab_frame)
            elif tab_name == "discord":
                discord_bot_tab.create_discord_bot_tab(new_tab_frame, self)
            elif tab_name == "advanced":
                advanced_tab.build(self, new_tab_frame)
            elif tab_name == "stats":
                stats_tab.build(self, new_tab_frame)
            elif tab_name == "logging":
                logging_tab.build(self, new_tab_frame)
            elif tab_name == "auto_setup":
                auto_setup_tab.build(self, new_tab_frame)
            else:
                # Unknown tab - show error
                error_label = ctk.CTkLabel(
                    new_tab_frame,
                    text=f"❌ Unknown tab: {tab_name}",
                    font=("Segoe UI", 14),
                    text_color="#ff4444",
                )
                error_label.pack(pady=50)

            # Pre-render everything (still invisible)
            new_tab_frame.update_idletasks()

            # NOW destroy old content quickly
            for widget in self.content_frame.winfo_children():
                if widget != new_tab_frame:
                    widget.destroy()

            # Pack new frame (instant since it's already rendered)
            new_tab_frame.pack(fill="both", expand=True)

            # Update sidebar highlight
            self.sidebar.select_item(tab_name)

            logger.info(f"Loaded tab: {tab_name}")

        except Exception as e:
            logger.error(f"Error loading tab '{tab_name}': {e}")
            # On error, clean up and show error
            new_tab_frame.destroy()
            error_frame = ctk.CTkFrame(self.content_frame, fg_color="transparent")
            error_frame.pack(fill="both", expand=True)
            error_label = ctk.CTkLabel(
                error_frame,
                text=f"❌ Error loading tab: {str(e)}",
                font=("Segoe UI", 12),
                text_color="#ff4444",
            )
            error_label.pack(pady=50)
    ...
    def toggle_start(self):
        ...
        # If auto setup is running, cancel it
        if self.auto_setup_running:
            if self.auto_setup_instance:
                self.auto_setup_instance.stop()
            ...
            if hasattr(self, "info_label") and self.info_label.winfo_exists():
                self.info_label.configure(
                    text="⏹ Auto Setup cancelled", text_color="#ff4444"
                )
            return
        ...
        # Check if Auto Setup should run first
        if self.auto_setup_enabled and not self.auto_setup_running:
            self._run_auto_setup_then_start()
            return

        self._actually_start_macro()

    def _run_auto_setup_then_start(self):
        """Run Auto Setup in a background thread, then start macro on success"""
        import threading

        self.auto_setup_running = True
        if hasattr(self, "status_label") and self.status_label.winfo_exists():
            self.status_label.configure(
                text="Status: 🚀 AUTO SETUP", text_color="#5a9bcf"
            )
        if hasattr(self, "start_button") and self.start_button.winfo_exists():
            self.start_button.configure(text="⏹ CANCEL")
        if hasattr(self, "info_label") and self.info_label.winfo_exists():
            self.info_label.configure(
                text="🚀 Auto Setup in progress...", text_color="#5a9bcf"
            )

        # Minimizar janela principal e mostrar HUD (igual ao macro normal)
        self.root.iconify()
        self.show_hud()
        ...
        self.auto_setup_instance = AutoSetup(
            mouse_controller=self.mouse,
            keyboard_controller=self.keyboard,
            color_detector=self.color_detector,
            window_manager=self.window,
            settings=auto_setup_settings,
        )

        # Set status callback to update GUI on main thread
        def on_status(msg):
            try:
                self.root.after(0, lambda: self._update_auto_setup_status(msg))
            except Exception:
                pass

        self.auto_setup_instance.set_status_callback(on_status)

        # Run in background thread
        def auto_setup_thread():
            try:
                success = self.auto_setup_instance.run()
                self.root.after(0, lambda: self._on_auto_setup_complete(success))
            except Exception as e:
                logger.error(f"Auto Setup thread error: {e}", exc_info=True)
                self.root.after(0, lambda: self._on_auto_setup_complete(False))

        t = threading.Thread(
            target=auto_setup_thread, name="AutoSetupThread", daemon=True
        )
        t.start()

    def _update_auto_setup_status(self, message):
        """Update GUI with auto setup status (called on main thread)"""
        try:
            # Update main GUI info label
            if hasattr(self, "info_label") and self.info_label.winfo_exists():
                self.info_label.configure(text=message, text_color="#5a9bcf")

            # Update auto setup tab status if visible
            if (
                hasattr(self, "auto_setup_status_label")
                and self.auto_setup_status_label.winfo_exists()
            ):
                self.auto_setup_status_label.configure(text=f"Status: {message}")

            # Update current_status for mini HUD (thread-safe)
            self.set_status(message)
        except Exception:
            pass

    def _on_auto_setup_complete(self, success):
        """Handle auto setup completion (called on main thread)"""
        self.auto_setup_running = False
        self.auto_setup_instance = None

        if success:
            logger.info("[AutoSetup] Complete! Starting fishing macro...")
            if hasattr(self, "info_label") and self.info_label.winfo_exists():
                self.info_label.configure(
                    text="✅ Auto Setup done! Starting macro...", text_color="#5a9bcf"
                )
            # Start the actual macro automatically after auto setup completes
            self.root.after(500, self._actually_start_macro)
        else:
            logger.warning("[AutoSetup] Failed or cancelled")
            # Restaurar janela principal e esconder HUD
            self.root.deiconify()
            self.hide_hud()
            if hasattr(self, "status_label") and self.status_label.winfo_exists():
                self.status_label.configure(
            ...
            if hasattr(self, "info_label") and self.info_label.winfo_exists():
                self.info_label.configure(
                    text="❌ Auto Setup failed", text_color="#ff4444"
                )
    ...
    def _actually_start_macro(self):
        ...
        if self.running:
            ...
            self.root.iconify()
            self.show_hud()
            # Garantir que o loop de atualização da HUD continue após auto setup
    ...
    def refresh_stats_display(self):
        ...
        try:
            ...
            today = stats_manager.get_today_summary()
            if (
                hasattr(self, "today_sessions_label")
                and self.today_sessions_label.winfo_exists()
            ):
                self.today_sessions_label.configure(text=str(today["sessions"]))
            if (
                hasattr(self, "today_fish_label")
                and self.today_fish_label.winfo_exists()
            ):
                self.today_fish_label.configure(text=str(today["fish"]))
            if (
                hasattr(self, "today_fruits_label")
                and self.today_fruits_label.winfo_exists()
            ):
            ...
            # Update history
            if (
                hasattr(self, "history_container")
                and self.history_container.winfo_exists()
            ):
    ...
    def _trigger_discord_menu_refresh(self):
        ...
        if (
            not self._discord_menu_message
            or not self.bot_service
            or not self.bot_service.is_running()
        ):
            return
    ...
    def update_hud(self):
        ...
        if self.hud_window is not None:
            try:
                # Se auto setup está rodando, mostrar status do auto setup
                if self.auto_setup_running:
                    if (
                        hasattr(self, "hud_time_label")
                        and self.hud_time_label.winfo_exists()
                    ):
                        self.hud_time_label.config(text="AUTO SETUP", fg="#5a9bcf")
                    if (
                        hasattr(self, "hud_fish_label")
                        and self.hud_fish_label.winfo_exists()
                    ):
                        self.hud_fish_label.config(text="")
                    if (
                        hasattr(self, "hud_fruits_label")
                        and self.hud_fruits_label.winfo_exists()
                    ):
                        self.hud_fruits_label.config(text="")
                    if (
                        hasattr(self, "hud_fish_rate_label")
                        and self.hud_fish_rate_label.winfo_exists()
                    ):
                        self.hud_fish_rate_label.config(text="")
                    if (
                        hasattr(self, "hud_fruit_rate_label")
                        and self.hud_fruit_rate_label.winfo_exists()
                    ):
                        self.hud_fruit_rate_label.config(text="")
                    if (
                        hasattr(self, "hud_fps_label")
                        and self.hud_fps_label.winfo_exists()
                    ):
                        self.hud_fps_label.config(text="")

                    # Mostrar status do auto setup na linha de atividade
                    if (
                        hasattr(self, "hud_activity_label")
                        and self.hud_activity_label.winfo_exists()
                    ):
                        with self.status_lock:
                            current_status = self.current_status
                        activity = (
                            current_status[:30] + "..."
                            if len(current_status) > 30
                            else current_status
                        )
                        self.hud_activity_label.configure(text=activity)

                    # Status dot azul durante auto setup
                    if (
                        hasattr(self, "hud_status_dot")
                        and self.hud_status_dot.winfo_exists()
                    ):
                        self.hud_status_dot.configure(fg="#5a9bcf")

                    # Continuar atualizando a cada 500ms
                    self.root.after(500, self.update_hud)
                    return

                # Se não está rodando ainda, continuar o loop mas não atualizar stats
                if not self.running:
                    self.root.after(500, self.update_hud)
                    return
                ...
                # Update status dot color - blue for auto setup, yellow if paused, green if running, red if stopped
                if (
                    hasattr(self, "hud_status_dot")
                    and self.hud_status_dot.winfo_exists()
                ):
            ...
            if (
                ...
                # Se auto setup está rodando, mostrar mensagem do auto setup
                if self.auto_setup_running:
                    # Buscar última mensagem de status do auto setup
                    with self.status_lock:
                        current_status = self.current_status
                    activity = (
                        current_status[:30] + "..."
                    ...
                    # Modo normal: mostrar status do macro
                    with self.status_lock:
                        current_status = self.current_status
    ...
    # ========== AUTO SETUP METHODS (v2.0.0) ==========

    def toggle_auto_setup(self):
        """Toggle auto setup on/off"""
        self.auto_setup_enabled = self.auto_setup_enabled_var.get()
        self.save_auto_setup_settings_from_ui()
        logger.info(
            f"Auto Setup: {'ENABLED' if self.auto_setup_enabled else 'DISABLED'}"
        )

    def start_auto_setup_coord_selection(self, coord_name):
        """Start coordinate selection for auto setup"""
        self.selecting_auto_setup_coord = coord_name

        # Map coord_name to UI short names
        name_map = {
            "auto_setup_play_button": ("play", "Play Button"),
            "auto_setup_private_server_1": ("ps1", "Private Server 1"),
            "auto_setup_vip_code_input": ("input", "VIP Code Input"),
            "auto_setup_enter_regular": ("enter", "Enter Regular"),
        }

        short, desc = name_map.get(coord_name, ("unknown", "Unknown"))

        # Update button/label if they exist
        btn_attr = f"auto_setup_{short}_btn"
        lbl_attr = f"auto_setup_{short}_label"
        if hasattr(self, btn_attr) and hasattr(self, lbl_attr):
            btn = getattr(self, btn_attr)
            lbl = getattr(self, lbl_attr)
            if btn.winfo_exists():
                btn.configure(text="Waiting...")
            if lbl.winfo_exists():
                lbl.configure(
                    text=f"RIGHT-CLICK on {desc}", text_color=self.warning_color
                )

        # Start mouse listener for right-click
        self._auto_setup_mouse_listener = MouseListener(
            on_click=self._on_mouse_click_auto_setup_coord
        )
        self._auto_setup_mouse_listener.start()

    def _on_mouse_click_auto_setup_coord(self, x, y, button, pressed):
        """Handle mouse clicks during auto setup coordinate selection"""
        if not self.selecting_auto_setup_coord:
            return

        if pressed and button == Button.right:
            coords = {"x": x, "y": y}
            coord_name = self.selecting_auto_setup_coord

            # Save coordinate to instance
            setattr(self, coord_name, coords)

            # Map coord_name to UI short names
            name_map = {
                "auto_setup_play_button": "play",
                "auto_setup_private_server_1": "ps1",
                "auto_setup_vip_code_input": "input",
                "auto_setup_enter_regular": "enter",
            }
            short = name_map.get(coord_name, "unknown")
            ...
            # Cleanup
            self.selecting_auto_setup_coord = None
            self._auto_setup_mouse_listener.stop()
            logger.info(f"Auto Setup coord '{coord_name}' set to ({x}, {y})")

    def save_auto_setup_settings_from_ui(self):
        """Save auto setup settings from UI to file"""
        try:
            settings = {
                "enabled": self.auto_setup_enabled,
                "vip_code": (
                    self.auto_setup_vip_code_entry.get()
                    if hasattr(self, "auto_setup_vip_code_entry")
                    and self.auto_setup_vip_code_entry.winfo_exists()
                    else self.auto_setup_vip_code
                ),
                "max_retries": (
                    int(self.auto_setup_max_retries_entry.get())
                    if hasattr(self, "auto_setup_max_retries_entry")
                    and self.auto_setup_max_retries_entry.winfo_exists()
                    else self.auto_setup_max_retries
                ),
                "game_open_timeout": (
                    int(self.auto_setup_game_open_timeout_entry.get())
                    if hasattr(self, "auto_setup_game_open_timeout_entry")
                    and self.auto_setup_game_open_timeout_entry.winfo_exists()
                    else self.auto_setup_game_open_timeout
                ),
                "game_load_timeout": (
                    int(self.auto_setup_game_load_timeout_entry.get())
                    if hasattr(self, "auto_setup_game_load_timeout_entry")
                    and self.auto_setup_game_load_timeout_entry.winfo_exists()
                    else self.auto_setup_game_load_timeout
                ),
                "play_button": self.auto_setup_play_button,
                "private_server_1": self.auto_setup_private_server_1,
                "vip_code_input": self.auto_setup_vip_code_input,
                "enter_regular": self.auto_setup_enter_regular,
                "menu_color_check": self.auto_setup_menu_color_check,
                "game_loaded_color_check": self.auto_setup_game_loaded_color_check,
            }
            # Update in-memory values
            self.auto_setup_vip_code = settings["vip_code"]
            self.auto_setup_max_retries = settings["max_retries"]
            self.auto_setup_game_open_timeout = settings["game_open_timeout"]
            self.auto_setup_game_load_timeout = settings["game_load_timeout"]
            # Update color tolerance if widget exists
            if (
                hasattr(self, "auto_setup_color_tolerance_entry")
                and self.auto_setup_color_tolerance_entry.winfo_exists()
            ):
                tolerance = int(self.auto_setup_color_tolerance_entry.get())
                self.auto_setup_menu_color_check["tolerance"] = tolerance
                self.auto_setup_game_loaded_color_check["tolerance"] = tolerance

            self.settings.save_auto_setup_settings(settings)
            logger.info("Auto Setup settings saved")
        except Exception as e:
            logger.error(f"Failed to save auto setup settings: {e}")

    def start_auto_setup_color_selection(self, color_type):
        """Start color selection for auto setup (menu or game_loaded)

        Args:
            color_type: "menu" or "game_loaded"
        """
        self.selecting_auto_setup_color = color_type

        # Update button text
        if color_type == "menu":
            btn = self.auto_setup_menu_color_btn
            status = self.auto_setup_menu_color_status
            desc = "Menu Color"
        else:
            btn = self.auto_setup_game_loaded_color_btn
            status = self.auto_setup_game_loaded_color_status
            desc = "Game Loaded Color"

        if btn.winfo_exists():
            btn.configure(text="Waiting...")
        if status.winfo_exists():
            status.configure(
                text=f"RIGHT-CLICK on pixel", text_color=self.warning_color
            )

        # Start mouse listener for right-click
        self._auto_setup_color_mouse_listener = MouseListener(
            on_click=self._on_mouse_click_auto_setup_color
        )
        self._auto_setup_color_mouse_listener.start()

    def _on_mouse_click_auto_setup_color(self, x, y, button, pressed):
        """Handle mouse clicks during auto setup color selection"""
        if not self.selecting_auto_setup_color:
            return

        if pressed and button == Button.right:
            color_type = self.selecting_auto_setup_color

            try:
                # Capture pixel color at clicked position
                import mss
                ...
                with mss.mss() as sct:
                    ...
                    pixel = screenshot.pixel(0, 0)  # Returns (R, G, B) tuple
                    color_rgb = pixel[:3]  # Get only RGB, ignore alpha

                coord = {"x": x, "y": y}

                # Save to appropriate color check dict
                if color_type == "menu":
                    self.auto_setup_menu_color_check["coordinate"] = coord
                    self.auto_setup_menu_color_check["color_rgb"] = color_rgb
                    btn = self.auto_setup_menu_color_btn
                    status = self.auto_setup_menu_color_status
                    preview = self.auto_setup_menu_color_preview
                else:
                    self.auto_setup_game_loaded_color_check["coordinate"] = coord
                    self.auto_setup_game_loaded_color_check["color_rgb"] = color_rgb
                    btn = self.auto_setup_game_loaded_color_btn
                    status = self.auto_setup_game_loaded_color_status
                    preview = self.auto_setup_game_loaded_color_preview

                # Update UI on main thread
                def update_ui():
                    if btn.winfo_exists():
                        btn.configure(text="Select Color")
                    if status.winfo_exists():
                        status.configure(
                            text=f"\u2713 X:{x} Y:{y} RGB{color_rgb}",
                            text_color=self.success_color,
                        )
                ...
                logger.info(
                    f"Auto Setup {color_type} color set to RGB{color_rgb} at ({x}, {y})"
                )
            ...
            # Cleanup
            self.selecting_auto_setup_color = None
            self._auto_setup_color_mouse_listener.stop()

    def test_auto_setup_detection(self):
        """Test both color detections to determine current state (menu/in-game/neither)"""
        try:
            result_label = self.auto_setup_detection_result

            # Get tolerance
            tolerance = self.auto_setup_menu_color_check.get("tolerance", 20)

            # Check menu color
            menu_check = self.auto_setup_menu_color_check
            menu_coord = menu_check.get("coordinate")
            menu_target = menu_check.get("color_rgb")
            menu_detected = False

            if menu_coord and menu_target:
                import mss
                ...
                menu_detected = self.color_detector.check_color_match(
                    current_color, menu_target, tolerance
                )

            # Check game loaded color
            game_check = self.auto_setup_game_loaded_color_check
            game_coord = game_check.get("coordinate")
            game_target = game_check.get("color_rgb")
            game_detected = False

            if game_coord and game_target:
                import mss
                ...
                with mss.mss() as sct:
                    ...
                    screenshot = sct.grab(monitor)
                    current_color = screenshot.pixel(0, 0)[:3]
                game_detected = self.color_detector.check_color_match(
                    current_color, game_target, tolerance
                )

            # Determine state and update UI
            if result_label.winfo_exists():
                if menu_detected and game_detected:
                    result_label.configure(
                        text="⚠️ Both colors detected (check configuration)",
                        text_color=self.warning_color,
                    )
                    logger.warning("Both menu and game colors detected simultaneously")
                elif menu_detected:
                    result_label.configure(
                        text="✓ IN MENU - Ready to join game",
                        text_color=self.success_color,
                    )
                    logger.info("Currently in menu screen")
                elif game_detected:
                    result_label.configure(
                        text="✓ IN-GAME - Game loaded successfully",
                        text_color=self.success_color,
                    )
                    ...
                    if (
                        ...
                        result_label.configure(
                            text="✗ Colors not configured yet",
                            text_color=self.error_color,
                        )
                    else:
                        result_label.configure(
                            text="✗ Neither color detected (wrong screen or adjust tolerance)",
                            text_color=self.error_color,
                        )
                        logger.warning("No color match detected")

        except Exception as e:
            logger.error(f"Failed to test detection: {e}")
            if (
                hasattr(self, "auto_setup_detection_result")
                and self.auto_setup_detection_result.winfo_exists()
            ):
                self.auto_setup_detection_result.configure(
                    text=f"✗ Test failed: {str(e)[:40]}",
                    text_color=self.error_color,
                )
